...
model_fit_test <- function(trainset, testsets, n_tr = 10000, n_ts = 2000, n_v = 2000, f = 32, classes = 10){
  data_dir = trainset
  ...
  print("Start Model fitting on MNIST ...")
  for(i in 1:classes){
    ...
    gpisep <- newGPsep(X.norm, y, d=rep(0.1, ncol(X.norm)),
                       g=eps, dK=TRUE)
    
    mleGPsep(gpisep, param="d", tmin=0.01, tmax=0.3)
...
score_function <- function(trainset, testset, q_ = 0.95, f = 32, n_tr = 100000, n_ts = 2000, n_v = 2000, classes = 10){
...
f = 32
InD_Dataset = "mnist-32"
OOD_Datasets = c("CIFAR10", "FashionMNIST", "Imagenet-c", "SVHN")
n_tr = 10000
n_ts = 2000
n_v = 2000
classes = 10

# Run only once for fitting, then the Rdata will be solved in Rdata_ckpt
